...
def inject_pins(index_html, sites, faces, summary, area_name):
    """Second, lighter layer: one pin per location (centroid), for an alternative footprint."""
    fp = summary["footprint_ft"]
    F = {f["properties"]["id"]: f["properties"] for f in faces}
    pins = []
    for f in sites:
        ring = f["geometry"]["coordinates"][0]
        lon = sum(c[0] for c in ring[:4]) / 4; lat = sum(c[1] for c in ring[:4]) / 4
        p = f["properties"]; fa = F.get(p["face"], {})
        pins.append([round(lat, 6), round(lon, 6), p["street"], fa.get("width_ft"), p.get("zone"), p.get("cat"), p.get("placement")])
    payload = {"footprint_ft": fp, "total": summary["total_locations"], "area": area_name, "pins": pins}
    block = ("/* CAMPABLE_PINS:BEGIN \u2014 generated by tools/campable_sites.py --pins, do not edit by hand */\n"
             "var CAMPABLE_PINS = " + json.dumps(payload, separators=(",", ":")) + ";\n"
             "/* CAMPABLE_PINS:END */")
    with open(index_html, encoding="utf-8") as fh:
        html = fh.read()
    pat = re.compile(r"/\* CAMPABLE_PINS:BEGIN.*?\*/.*?/\* CAMPABLE_PINS:END \*/", re.S)
    if not pat.search(html):
        raise SystemExit("no CAMPABLE_PINS:BEGIN/END markers in index.html")
    html = pat.sub(lambda m: block, html, count=1)
    with open(index_html, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("injected %d pins (%gx%g ft) into index.html" % (len(pins), fp[0], fp[1]), file=sys.stderr)
...
def main():
    ...
    ap.add_argument("--pins", action="store_true",
                    help="also write the result as a pin layer (CAMPABLE_PINS block in index.html); use with --footprint")
    args = ap.parse_args()
    ...
    if args.pins:
        inject_pins(os.path.join(repo, "index.html"), sites, faces, summary, args.area_name)
